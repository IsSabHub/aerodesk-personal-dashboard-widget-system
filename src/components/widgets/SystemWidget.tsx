  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics(prev => ({
        ...
        ram: Math.max(40, Math.min(90, prev.ram + (Math.random() - 0.5) * 2)),
        network: Math.max(1, Math.min(100, prev.network + (Math.random() - 0.5) * 5)),
    ...
    <WidgetCard 
      ...
      actions={
        <div className="flex items-center gap-1">
          ...
          <span className="text-[9px] font-black text-yellow-400 uppercase tracking-tighter">Peak Performance</span>
        </div>
      }
    >
        ...
        <div className="pt-4 border-t border-white/5">
          <div className="flex items-center justify-between text-[10px] text-white/30 font-bold uppercase tracking-widest">
            ...
            <span className={cn(
              ...
              metrics.temp > 70 ? "text-red-400" : "text-green-400"
            )}>
